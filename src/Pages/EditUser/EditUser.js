...
export default function EditUser() {
  ...
  const [orgModified, setOrgModified] = useState(false);
  ...
  const closePendingError = () => setPendingUserError(false);
  ...
  return (
    <div className="wrapperBase">
      ...
      <Modal size="md" show={showWarning} onHide={handleClose} centered>
        <Modal.Body className="p-4">
          Important: You are mapping {user?.orgEmail.split('@')[0]}@<b>{user?.orgEmail.split('@')[1]}</b> to{' '}
          <b>{orgNameSelected.companyName}</b>. This will allow {user?.firstName} {user?.lastName} access to information related
          to <b>{orgNameSelected.companyName}</b>. Are you sure that this is correct ?
        </Modal.Body>
        <Modal.Footer className="p-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              setUser((prevState) => ({ ...prevState, orgName: orgNameSelected.companyName, orgNo: orgNameSelected.customerNo }));
      ...
      <Modal size="md" show={pendingUserError} onHide={closePendingError} centered>
        <Modal.Body className="p-4">
          Important: You are mapping {user?.orgEmail.split('@')[0]}@<b>{user?.orgEmail.split('@')[1]}</b> to{' '}
          <b>{user?.orgName}</b>. This will allow {user?.firstName} {user?.lastName} access to information related to{' '}
          <b>{user?.orgName}</b>. Are you sure that this is correct ?
        </Modal.Body>
        <Modal.Footer className="p-3">
          <Button variant="secondary" onClick={closePendingError}>
            Cancel
          </Button>
          <Button
            variant="primary"
      ...
      <div className="wrapperCard">
        ...
        <div className="wrapperCard--body">
          <div className="editWrap">
            <Form onSubmit={handleSubmit(editUser)} id="editUserForm">
              <div className="mb-3">
                <Form.Label>
