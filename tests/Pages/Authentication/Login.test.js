...
describe('Login page', () => {
  ...
  it('should render app links', async () => {
    const playStore = screen.getByTestId('playStoreURL');
    const appStore = screen.getByTestId('appStoreURL');
    const playStoreImg = screen.getByAltText(/Play store/i);
    const appStoreImg = screen.getByAltText(/App store/i);
    expect(playStore).toHaveAttribute('href', 'https://play.google.com/');
    expect(appStore).toHaveAttribute('href', 'https://www.apple.com/in/app-store/');
